...
export const videoApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    createVideo: builder.mutation<
      { message: string; data: Video; url: string },
      { courseId: string; body: CreateVideoRequest }
    >({
      query: ({ courseId, body }) => ({
        // url: `courses/${courseId}/videos`,
        url: API.VIDEO.GET_COURSE_VIDEOS.replace(":courseId", courseId),
        method: "POST",
        data: body,
      }),
    ...
    getCourseVideoById: builder.query<
      { data: Video },
    ...
    updateVideo: builder.mutation<
      { message: string; data: Video },
      { id: string; body: UpdateVideoRequest }
    >({
      query: ({ id, body }) => ({
        url: `${API.VIDEO.ROUTE}/${id}`,
        method: "PATCH",
        data: body,
      }),
      invalidatesTags: (result, _error, { id }) => [
        ...
        ...(result
          ? [{ type: "Video" as const, id: `COURSE_${result.data.course_id}` }]
          : []),
      ],
    }),
